...
The first poll response was collected May 20, 2024. The collection is still ongoing as of 9/16/2024. To protect respondent anonymity, raw data is not yet available until approved by our IRB review.

A blank version of the poll can be viewed [here](https://drive.google.com/file/d/134MgEsKRB3GohHQTUAdQMruNI3yXmIwF/view?usp=drive_link).
...
library(SnowballC) # Snowball Stemmers Based on the C 'libstemmer' UTF-8 Library CRAN v0.7.1 # Snowball Stemmers Based on the C 'libstemmer' UTF-8 Library CRAN v0.7.1 
library(scales) # Scale Functions for Visualization CRAN v1.3.0 # Scale Functions for Visualization CRAN v1.3.0 
...
rsch_breakdown <-
  clean_types_dat %>% select(r1:cc) %>% rename(
    `R1 institution` = r1,
    `R2 institution` = r2,
...
```{r}
do_col_plot("familiar") & scale_y_continuous(limits = c(0, 5), oob = scales::oob_squish) 
```
...
```{r}
do_yn_plot("done_research", legend_title = "") # legend_title = "Have you conducted microbiome research, previously or currently?"
```
...
```{r}
do_yn_plot("offer_courses", legend_title = "") # legend_title = "Specific courses or educational programs for students related to microbiomes"
```
...
```{r}
do_col_plot("want_collab") & scale_y_continuous(limits = c(0, 5), oob = scales::oob_squish) 
```
...
```{r}
do_col_plot("helps_students") & scale_y_continuous(limits = c(0, 5), oob = scales::oob_squish) 
```
...
```{r}
do_col_plot("self_confidence") & scale_y_continuous(limits = c(0, 5), oob = scales::oob_squish)
```
...
```{r}
do_col_plot("dev_opportunities") & scale_y_continuous(limits = c(0, 5), oob = scales::oob_squish)
```
...
```{r}
do_col_plot("comp_access") & scale_y_continuous(limits = c(0, 5), oob = scales::oob_squish)
```
...
```{r}
do_col_plot("lab_access") & scale_y_continuous(limits = c(0, 5), oob = scales::oob_squish)
```
...
```{r}
do_col_plot("time_financial") & scale_y_continuous(limits = c(0, 5), oob = scales::oob_squish)
```
...
```{r}
do_col_plot("materials_access") & scale_y_continuous(limits = c(0, 5), oob = scales::oob_squish)
```
...
```{r}
do_col_plot("course_freedom") & scale_y_continuous(limits = c(0, 5), oob = scales::oob_squish)
```
...
```{r}
do_col_plot("student_interest") & scale_y_continuous(limits = c(0, 5), oob = scales::oob_squish)
```
...
```{r eval = F}
do_challenge_plot_binned <- function() {
  rsch_dat<-
    clean_types_dat %>%
    select(7:14, research_type) %>%
    pivot_longer(!research_type) %>%
    mutate(name = case_when(
      name == "self_confidence" ~ "instructor",
      name == "dev_opportunities" ~ "instructor",
      name == "materials_access" ~ "access",
      name == "comp_access" ~ "access",
      name == "lab_access" ~ "access",
      name == "time_financial" ~ "time",
      name == "course_freedom" ~ "institutional",
      name == "student_interest" ~ "student"
    )) %>% 
    group_by(research_type, name) %>%
    summarize(mean = mean(value), sd = sd(value)) %>%
    mutate(type = case_when(
  ...
  msi_dat<-
    ...
    mutate(name = case_when(
      name == "self_confidence" ~ "instructor",
      name == "dev_opportunities" ~ "instructor",
      name == "materials_access" ~ "instructor",
      name == "comp_access" ~ "institutional",
      name == "lab_access" ~ "institutional",
      name == "time_financial" ~ "institutional",
      name == "course_freedom" ~ "institutional",
      name == "student_interest" ~ "student"
    )) %>% 
    group_by(msi_type, name) %>%
  ...
  gg_out <-
    ...
    mutate(name = case_when(
      name == "institutional" ~ "Institutional access or lack of flexibility",
      name == "instructor" ~ "Limited materials, confidence, or professional dev opps",
      name == "student" ~ "Student interest and/or engagement"
    ) 
    ) %>% 
    ggplot(aes(
      x = name,
...
# mean(stats_dat$value)
# var(stats_dat$value)

summary(glm(data = stats_dat, value ~ name + research_type + msi_type, family = poisson(link = "log")))
```
...
words_dat %>%
  ...
  labs(
    y = "Undergraduate institution",
    x = "Graduate institution"
  )
```
...
> _Some thoughts for the authors: we could propose integrating into a microbiology lab course, or a short research project, students can focus on different microbiome systems (soil, viruses, archaea, etc), could propose fitting into a CURE. For example, if studying soil, could look at the virome or archaea in soil. Would be a good complement to lectures on microbiome that are part of microbiology course._

> _might need to think about technical limitations for different -omes (fungal, bacterial..). DNA extraction might be an issue for experiments, depends on how many students, what interests, etc. Individual instructor will need to own it and decide what they are interested in, and how they can best incorporate it -- eg., give a fungi twist if that's what you're passionate about! Variety of ways to implement. Could divide up by time - first half of the semester is bact/archaea, while second half is others._

> _Could be included in introductory biology lab as well. Termite gut, soil... half semester research project at VSU. **ACTION:** Xianfa could create a short write up about a microbiome education experience_